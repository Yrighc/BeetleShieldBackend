...
- 加固后 = `100 - Σ(已生效维度权重)`，最低限制在 5 分（不允许出现 0 分"绝对安全"的误导性展示）。
...
**总分权重表有 6 类，但前端柱状图只展示 5 个维度**（没有单独的"Hook/注入防御"一栏）。展示层把"反调试/环境检测"和"Hook/注入防御"合并成前端的"反调试保护"一项（合并权重 30，取两者得分之和 / 30 换算百分比），其余 4 项一一对应：DEX 混淆、SO 加壳保护、资源/字符串加密→"资源文件加密"、签名校验。总分计算仍按上表 6 类独立权重相加，不受展示层合并影响。
